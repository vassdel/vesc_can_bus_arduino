...
int index(canid_t x){       // mapping of HEX codes to int indexes for the arrays
  ...
  if (x == 0x80001073) return  4; // motorMosfetTemp, motorTemp, motorInputCurrent, (pid)
  if (x == 0x80000973) return  1; // motorRpm , motorCurrent, motorDutyCycle
  if (x == 0x80000E73) return  2; // motorConsumedAh, motorChargedAh
  if (x == 0x80000F73) return  3; // motorConsumedWh, motorChargedWh
...
void check_all_data(){                // if we have all the data (7 info) for this short time it delays() and sets is_found[] = true; 
  for(int i = 0; i<7; i++){           // change 1 to 0 for BMS
...
>>>>>>> 0925d738f0feae5f0645c1ed90a99e787ee5e060
}