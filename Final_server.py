...
'''Για να τρέξουμε αρχικά τον σερβερ, τρεχουμε το αρχειο ngrok.exe και του δινουμε την εντολη ngrok tcp 8080 or ngrok tcp --region=eu --remote-addr=8.tcp.eu.ngrok.io:20375 22
    ...
    while True:
        ...
        if len(my_str)<=600:
            if len(my_str)>=150:
                ...
                with open('serverdata.csv', 'a') as csv_file: #open serverdata in append mode, keep appending to the csv
                    csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                #ta dedomena gia ta headers
                #na prosthesw sto data to time
                    csv_writer.writerow(data2) #writes one row at a time sto serverdata.csv
                    csv_file.close()
                if not data:
                    break
                print(f"{data.decode()}")
